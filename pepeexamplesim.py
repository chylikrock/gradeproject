...
def generate_pepe_price_data(days: int = 100) -> tuple:
    """
    Генерация тестовых данных цен для PEPE
    """
    start_price = 0.00001
    ...
    for i in range(1, days):
        volatility = random.uniform(0.02, 0.15) if i < 30 else random.uniform(0.05, 0.3)

        if i < 40:
            trend = random.uniform(0.95, 1.08)
        elif i < 70:
            trend = random.uniform(0.92, 1.05)
        else:
            trend = random.uniform(0.88, 1.02)

        new_close = close_prices[-1] * trend

        new_high = new_close * (1 + random.uniform(0.01, volatility))
        new_low = new_close * (1 - random.uniform(0.01, volatility))
...
def plot_results(
    ...
    if macd_line and signal_line and histogram:
        ...
        for i in range(len(macd_display)):
            ...
            if macd_val is None or signal_val is None or hist_val is None:
                ...
                macd_str = f"{macd_val:+.8f}"
                signal_str = f"{signal_val:+.8f}"
                hist_str = f"{hist_val:+.8f}"
    ...
    for i, mom in enumerate(momentum_display):
        ...
        day_num = start_idx + i + 1
        mom_str = f"{mom:.8f}" if mom is not None else "      N/A     "
    ...
    for i in range(len(bull_display)):
        ...
        if bull is None or bear is None:
            ...
            if bull > 0:
                bull_strength = "█" * min(10, int(abs(bull) * 1000))
            if bear < 0:
                bear_strength = "█" * min(10, int(abs(bear) * 1000))
...
def main():
    ...
    print("=" * 60)
    print("Тестирование индикаторов для PEPE (с RSI и MACD)")
    print("=" * 60)

    # Ген тест
    days = 100
    close_prices, high_prices, low_prices = generate_pepe_price_data(days)

    print(f"\nСгенерировано {days} дней данных для PEPE")
    print(f"Первые 5 значений закрытия: {[f'{p:.8f}' for p in close_prices[:5]]}")
    print(f"Последние 5 значений закрытия: {[f'{p:.8f}' for p in close_prices[-5:]]}")
    print(f"Минимальная цена: {min(close_prices):.8f}")
    print(f"Максимальная цена: {max(close_prices):.8f}")
    ...
    for i in range(start_idx, days):
        price_str = f"{close_prices[i]:.8f}"
        mom_str = f"{momentum_values[i]:.8f}" if momentum_values[i] is not None else "None"
        rsi_str = f"{rsi_values[i]:.2f}" if rsi_values[i] is not None else "None"
        macd_str = f"{macd_line[i]:.8f}" if macd_line[i] is not None else "None"
        hist_str = f"{histogram[i]:.8f}" if histogram[i] is not None else "None"
    ...
    for signal in mom_signals[-10:]:
        print(f"  День {signal[0] + 1}: {signal[1]} (значение: {signal[2]:.8f})")
    ...
    for signal in macd_signals[-10:]:
        print(f"  День {signal[0] + 1}: {signal[1]} (MACD: {signal[2]:.8f})")

    print(f"\nСигналы Bull Bear Power:")
    for signal in bbp_signals[-10:]:
        print(f"  День {signal[0] + 1}: {signal[1]} (быки: {signal[2]:.8f}, медведи: {signal[3]:.8f})")
    ...
    if valid_momentum:
        print(f"Momentum:")
        print(f"  Среднее: {sum(valid_momentum) / len(valid_momentum):.8f}")
        print(f"  Максимум: {max(valid_momentum):.8f}")
        print(f"  Минимум: {min(valid_momentum):.8f}")
    ...
    if valid_macd:
        print(f"\nMACD:")
        print(f"  Среднее: {sum(valid_macd) / len(valid_macd):.8f}")
        print(f"  Максимум: {max(valid_macd):.8f}")
        print(f"  Минимум: {min(valid_macd):.8f}")
    ...
    if valid_bull and valid_bear:
        ...
        print(f"  Среднее: {sum(valid_bull) / len(valid_bull):.8f}")

        print(f"\nBear Power:")
        print(f"  Отрицательных значений: {sum(1 for b in valid_bear if b < 0)}/{len(valid_bear)}")
        print(f"  Среднее: {sum(valid_bear) / len(valid_bear):.8f}")
    ...
    print(f"Начальная цена: {first_price:.8f}")
    print(f"Конечная цена:  {last_price:.8f}")
